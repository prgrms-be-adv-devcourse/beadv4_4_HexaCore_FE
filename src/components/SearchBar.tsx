...
export const SearchBar: React.FC<SearchBarProps> = ({
    ...
    return (
        <div className={`relative flex items-center w-full mx-auto px-5 ${className}`} style={{ maxWidth }}>
            ...
            />
            <button className="absolute right-7 top-1/2 -translate-y-1/2 flex h-11 w-11 items-center justify-center rounded-full bg-accent text-white border-none cursor-pointer transition-transform duration-200 hover:scale-105 active:scale-95 shadow-md">
                <Search size={20} />
            </button>
        </div>
    );
